...
# return the maximum interatomic distance between non-hydrogen atoms
def get_max_heavy_atoms_dist(Z_mol, xyz_mol):
    dist = []

    for i in range(len(Z_mol)):
        if Z_mol[i] != 1:
            for j in range(i+1, len(Z_mol)):
                if Z_mol[j] != 1:
                    a = xyz_mol[i] - xyz_mol[j]
