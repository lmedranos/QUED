...
def prepare_data(
    ...
    eg_dftb = newgap(p10)
    eg_dftb = np.abs(np.array(eg_dftb))
    target = np.array(target)
    
    dtarget = target - eg_dftb

    o1 = open('delta-egap.dat', 'w')
    for ii in range(len(target)):
      o1.write('{:>24}'.format(target[ii]) + '{:>24}'.format(eg_dftb[ii]) + '{:>24}'.format(dtarget[ii]) + '\n')
    o1.close()
