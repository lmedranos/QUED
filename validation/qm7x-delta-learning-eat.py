...
EDFTB_atom = {1:-7.4723, 6:-39.3067, 7:-60.3871, 8:-85.5034, 53:-70.3444,\
                9:-115.6543, 11:-2.7405, 12:-9.3976, 15:-46.1797, 16:-63.2426, \
                17:-84.3233, 19:-2.3186, 20:-7.5071, 30:-117.3181, 35:-79.7254 }
...
def prepare_data(
    ...
    for i in idxs:
        ...
        iZ = atoms.get_atomic_numbers()
        edftb.append(float(sum([ EDFTB_atom[zi] for zi in iZ ])))
        ...
        nchg = chg2glo(atoms.get_atomic_numbers(),properties['TBchg'])
        p11.append(nchg)
    
    p11 = _complete_list(p11)

    zip_dftb = zip(p4, p5, p6, p7, p8, edftb)
    e_atom_dftb = [x1 + x2 + x3 + x4 + x5 - x6 for (x1, x2, x3, x4, x5, x6) in zip_dftb]
    e_atom_dftb = np.abs(np.array(e_atom_dftb))
    target = np.array(target)
    
    dtarget = target - e_atom_dftb

    o1 = open('delta-eat.dat', 'w')
    for ii in range(len(target)):
      o1.write('{:>24}'.format(target[ii]) + '{:>24}'.format(e_atom_dftb[ii]) + '{:>24}'.format(dtarget[ii]) + '\n')
    o1.close()
    ...
    return dict(
        ...
        slatm=slatm_final,
        props=props_final,
        target_props=dtarget,
        idxs_to_permute=range(len(dtarget))
    )
...
def _train_parallel(
    ...
    # plt.close()
    # plt.scatter(maes_val, mses_val, c='r', label='val')
    # plt.scatter(maes_test, mses_test, c='b', label='test')
    # plt.xlabel('MAE')
    # plt.ylabel('RMSE')
    # plt.legend()
    # plt.savefig(f'{results_dir}/{dataset_name}'
    #             f'/plots/models_{n_train}_train'
    #             f'_{n_val}_val.jpeg', dpi=500)
    # plt.close()

    best_metrics, best_params = sorted_val_res_list[0]

    # plot_metrics(best_params['val_predictions'], best_params['test_predictions'],
    #              best_params['y_val'], best_params['y_test'],
    #              n_train, n_val, dataset_name, results_dir, lim=(y_min, y_max))
...
def main_qm_dataset(
    ...
    # _remove_create_dir(f'{results_dir}/{dataset_name}/plots/')
    ...
    # plot_performance(train_points, mse_val, mae_val,
