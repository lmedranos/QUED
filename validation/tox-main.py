...
ALLOWED_PROPERTIES = (
    "toxicity", "DMAX", "EFermi", "EBand", "NE", "Eh0", "Escc", "E3rd",
    "Erep3", "Embd", "dip", "Eig", "CHG"
)
...
def prepare_data(
    ...
    def _complete_list(prop_list):
        ...
        for prop in prop_list:
            element = np.concatenate(
                (prop, np.zeros((natoms - len(prop)))),
                axis=None
            ) if len(prop) != natoms else prop
            tmp.append(element)
    ...
    logger.info(
        f'train dataset size: {n} conformers; '
    ...
    if add_qm_props:
        ...
        elif prop_set=='electronic': 
            _vars += [p10]
            _vars_names += ['TBeig']
        elif prop_set=='atomic': 
            _vars += [p11]
            _vars_names += ['TBchg']
        logger.info(f'extracting {prop_set} properties: {_vars_names}')
    ...
    if bob:
        bob_repr = np.array([generate_bob(
            Z[mol], xyz[mol], size=196, atomtypes={'C', 'H', 'N', 'O', 'S', 'Cl', 'F', 'Br', 'I', 'P'},
    ...
    if props:
        logger.info(f'descriptor dimensions: {props_final.shape}')
    if cm:
...
def _perform_random_iter(
    ...
    rmse_val = root_mean_squared_error(y_val, yp_val)
    ...
    rmse_test = root_mean_squared_error(y_test, yp_test)
...
def get_parser():
    ...
    parser.add_argument('-y', '--target_property', type=str, default='toxicity',
                        help=f'Property to predict (case-insensitive). '
                             f'Sting type, default - "toxicity (toxicity)". ')
    ...
    parser.add_argument('-ps', '--prop_set', type=str, default='all',
...
if __name__ == '__main__':
    ...
    target_prop = arguments.target_property.lower()
